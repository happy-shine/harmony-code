...
# Usage: start
# Note: the `--gateway` flag was retained as a no-op for backward-compat
# with `make docker-start-pro` after LangGraph was removed in M5.
start() {
    local sandbox_mode
    local services

    # --gateway is accepted but silently ignored.
    for arg in "$@"; do
        if [ "$arg" = "--gateway" ]; then
            :
        fi
