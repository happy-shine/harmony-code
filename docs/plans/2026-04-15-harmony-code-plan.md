...
Note on hook frames: these tests verify the parser *extracts and passes through* hook frames correctly. The design decision (2026-04-15) is that hook frames are **dropped at the SSE gateway boundary**, not at the parser. Parser stays content-agnostic; filtering lives one layer up.
